...
{
public:
    imaginary_gaussian_distribution(std::complex<double> a_, uint n_autocorrelation_, uint n_initialization, const double epsilon, const int M_=0) :
        a(a_), n_autocorrelation(n_autocorrelation_),
        complex_gaussian_site_system_parameters(SiteParameters< std::complex<double>, ModelParams, ComplexLangevinUpdateParameters<ModelParams>, SiteSimpleUpdateParameters>(
                json {
                    ...
                    {ModelParams::param_file_name(), {{"a", a_}, {"b", {0.0, 0.0}}}},
