...
struct MemoryParallelUpdate : public UpdateDynamicsBase<MemoryParallelUpdate<SiteType>>
{
    ...
    public:
        explicit MeasureDetailedBalanceAccuracyPolicy(std::vector<SiteType> &prev_lattice_) : prev_lattice(prev_lattice_)
        {}
    ...
    std::vector< std::unique_ptr<common_measures::MeasurePolicy<SB>> > generate_update_dynamics_measures(const json& measure_names)
    {
        ...
        for (auto& measure_name :  measure_names)
            if(measure_name == "DetailedBalanceAccuracy")
                measures.push_back(std::make_unique<MeasureDetailedBalanceAccuracyPolicy<SB>>(previous_lattice));
        return measures;
    }
