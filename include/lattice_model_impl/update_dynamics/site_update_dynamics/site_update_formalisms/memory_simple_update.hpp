    ...
    std::vector< std::unique_ptr<common_measures::MeasurePolicy<SB>> > generate_update_dynamics_measures(const json& measure_names)
    {
        ...
        for (auto& measure_name :  measure_names)
            if(measure_name == "DetailedBalanceAccuracy")
                measures.push_back(std::make_unique<complex_monte_carlo_measures::MeasureDetailedBalanceAccuracyPolicy<SB>>(site_system_params, previous_site));
            else if(measure_name == "AbsoluteDetailedBalanceAccuracy")
                measures.push_back(std::make_unique<complex_monte_carlo_measures::MeasureAbsoluteDetailedBalanceAccuracyPolicy<SB>>(site_system_params, previous_site));
            else if(measure_name == "RealStepSize")
                measures.push_back(std::make_unique<complex_monte_carlo_measures::MeasureRealStepSizePolicy<SB>>(previous_site));
            else if(measure_name == "ImagStepSize")
                measures.push_back(std::make_unique<complex_monte_carlo_measures::MeasureImagStepSizePolicy<SB>>(previous_site));
            else if(measure_name == "ComplexStepSize")
                measures.push_back(std::make_unique<complex_monte_carlo_measures::MeasureComplexStepSizePolicy<SB>>(previous_site));
        return measures;
    }
