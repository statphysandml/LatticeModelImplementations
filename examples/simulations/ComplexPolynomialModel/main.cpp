...
void custom_main()
{
    ...
    // Second simulation with varying lambda_imag
    
    model_name = "ComplexPolynomialModelVaryingLambdaImag";
    rel_config_path = "/configs/" + model_name + "/";
    rel_data_path = "/data/" + model_name + "/";
    
    typedef mcmc::execution::ExpectationValueParameters ExecutionParams;
    ExecutionParams execution_parameters_varying_lambda_imag(100, 100000, 0, {}, // optional additional measures
                                         {"SecondMoment"}); // Meausures which will be evaluated in terms of mean and error evaluation

    auto simparams_varying_lambda_imag = mcmc::simulation::SimulationParameters< SystemBaseParams , ExecutionParams >::generate_simulation(
            site_parameters, execution_parameters_varying_lambda_imag,
           rel_data_path, "model_params", "lambda_imag", 0.0, 1.0, 11);
    simparams_varying_lambda_imag.write_to_file(rel_config_path);
